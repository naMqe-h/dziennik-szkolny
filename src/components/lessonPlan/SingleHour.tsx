...
import { singleHoursFromLessonPlan, SingleTeacherData, SubjectData } from '../../utils/interfaces'
...
export const SingleHour: React.FC<SingleHourProps> = ({ lesson, isEmpty }) => {
    ...
    const subjects = useSelector((state: RootState) => state.schoolData.schoolData?.subjects)
        ...
        <td className="border-2 border-base-200">
            <div>
                <h1 className="text-lg font-bold">
                    ...
                    {lesson?.subject === 'GodzinaWychowawcza' && 'Godzina wychowawcza'}
                </h1>
                <span className="text-sm text-gray-500">
                    {isEmpty ? '' : `${teacher?.lastName || ''} ${teacher?.firstName || ''}`}
                </span>
            </div>
        </td>
    )
