...
import { SingleClassData, SingleSubjectInClasses, SingleTeacherData, SubjectData } from "../../../utils/interfaces"
...
export const Subjects: React.FC<SubjectsProps> = ({ subjects, singleClass, isOpen, setIsOpen }) => {
    ...
    const [readySubjects, setReadySubjects] = useState<SubjectDataWithTeacher[]>()
        ...
        <>
            <div className={`modal ${isOpen ? 'modal-open' : ''} `}>
                <div className="modal-box flex flex-col items-center gap-2 bg-base-300">
                    ...
                    <h2 className="mb-3">Dodaj przedmiot dla klasy <span className="font-bold">{singleClass?.fullName}</span></h2>
                    Wybierz przedmiot:
                    <select onChange={(e) => setSubjectValue(e.currentTarget.value)} className="select select-bordered w-full max-w-xs">
                        <option value=""></option>
                        {newSubjects?.map(subject => (
                            <option value={subject.key} key={subject.name}>{subject.name}</option>
                        ))}
                    </select>
                    Wybierz nauczyciela uczącego:
                    <select onChange={(e) => setTeacherValue(e.currentTarget.value)} className="select select-bordered w-full max-w-xs">
                        <option value=""></option>
                        {availableTeachers?.map(teacher => (
                            <option value={teacher.email} key={teacher.email}>{teacher.lastName} {teacher.firstName}</option>
                        ))}
                    </select>
                    <button onClick={handleAdd} className="btn btn-primary w-44">Dodaj</button>
                </div>
            </div>
