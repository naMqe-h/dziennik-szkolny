...
export const LessonPlan: React.FC<LessonPlanProps> = ({ singleClass }) => {
    ...
    const allLessonPlans = useSelector((state: RootState) => state.schoolData.schoolData?.lessonPlans)

    useEffect(() => {
        if(singleClass && allLessonPlans) {
            setSingleClassLessonPlan(allLessonPlans[singleClass.name] as singleClassLessonPlan);
        }
